...
- Workplace dynamic contact network:
This data set contains the temporal network of contacts between N=92 individuals measured in an office building in France for two weeks (10 work days). The original data is publicly available from the SocioPatterns website http://www.sociopatterns.org/datasets/contacts-in-a-workplace/. The original data is a temporal network with temporal resolution of 20s, where the nodes represent the individuals and each temporal edge corresponds to the detection
...
- Hypertext 2009 (HT09) conference dynamic contact network:
This temporal network represents the face-to-face proximity interactions between N=100 conference attendees at the ACM Hypertext 2009 conference (2.5 days). The original data is a temporal network with temporal resolution of 20s, where the nodes represent the attendees and each temporal edge corresponds to the detection of a face-to-face contact between them. Here we only take into account the first day of the conference. The original data is publicly available from the SocioPatterns website http://www.sociopatterns.org/datasets/hypertext-2009-dynamic-contact-network/ .
